...
void initObjectBankFinish() {
    ...
    if( autoGenerateVariableObjects ) {
        ...
        FILE *f = fopen( "settings/bookID.txt", "r" );
        
        if( f != NULL ) {
            fscanf( f, "bookID=%d\nbookUniqueIdOffset=%d", &bookID, &bookUniqueIdOffset );
        }
        fclose( f );

        printf( "   bookID is %d\n", bookID );
        printf( "   bookUniqueIdOffset is %d\n", bookUniqueIdOffset );
        }
